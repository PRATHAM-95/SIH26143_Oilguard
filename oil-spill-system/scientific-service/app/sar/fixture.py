...
def build_fixture_scene(
    ...
    sea_level_db = -1.5
    amp_db = sea_level_db + _gaussian_field((rows, cols), seed=seed, scale_db=0.35)
    ...
    edge = _gaussian_field((rows, cols), seed=seed, scale_db=0.7) > 0.4
    ...
    scene = SarScene(
        scene_id=f"fixture-synth-{seed}",
        acquisition_time=datetime.fromisoformat(acquisition_iso.replace("Z", "+00:00"))
